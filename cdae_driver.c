...
int main() {
    printf("--- Khởi động CDAE Driver trên Kria KV260 ---\n");

    // 1. Mở /dev/mem để map memory vật lý (Cho PetaLinux/Ubuntu)
    int mem_fd = open("/dev/mem", O_RDWR | O_SYNC);
    if (mem_fd < 0) {
        perror("Không thể mở /dev/mem (Chạy bằng sudo?)");
        return -1;
    }
    ...
    printf("[+] Đã map phần cứng tại địa chỉ ảo: %p\n", cdae_base);

    // =========================================================
    // PHA 1: CHUẨN BỊ DỮ LIỆU VÀ NẠP XUỐNG FPGA
    // =========================================================
    printf("[+] Đang nạp tile 56x56x3 xuống FPGA RAM...\n");
    ...
    printf("[+] Nạp dữ liệu hoàn tất!\n");

    // =========================================================
    // PHA 2: RA LỆNH CHẠY INFERENCE
    // =========================================================
    printf("[+] Bắt đầu quá trình Inference (19 layers)...\n");
    ...
    printf("[+] Inference hoàn tất! Đang đọc kết quả...\n");
    ...
    printf("[+] Đọc hoàn tất. 5 pixel kết quả đầu tiên:\n");
    for (int i = 0; i < 5; i++) {
        printf("    Pixel[%d] = %f\n", i, output_tile[i]);
    }
