...
#define AXI_WRITE(base, offset, data)                                          \
  (*(volatile uint32_t *)((uint8_t *)(base) + (offset)) = (uint32_t)(data))
#define AXI_READ(base, offset)                                                 \
  ...
  for (int c = 0; c < IMG_C; c++) {
    for (int dy = 0; dy < TILE_H; dy++) {
      for (int dx = 0; dx < TILE_W; dx++) {
        ...
        int dst_idx = c * (TILE_H * TILE_W) + dy * TILE_W + dx;
  ...
  for (int c = 0; c < IMG_C; c++) {
    for (int dy = 0; dy < TILE_H; dy++) {
      for (int dx = 0; dx < TILE_W; dx++) {
        ...
        int src_idx = c * (TILE_H * TILE_W) + dy * TILE_W + dx;
...
int main() {
  ...
  void *cdae_base = mmap(NULL, CDAE_MAP_SIZE, PROT_READ | PROT_WRITE,
                         MAP_SHARED, mem_fd, CDAE_BASE_ADDR);
